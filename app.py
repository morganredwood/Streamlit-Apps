...
st.set_page_config(layout="wide", page_title="Executive Function Assistant", page_icon="🧠")
...
def get_available_lists(user_key: str):
    ...
    if not supabase or not user_key.strip():
        return []
    try:
        response = supabase.table("tasks_db").select("list_name").eq("user_key", user_key.strip()).execute()
        if response.data:
    ...
    try:
        ...
        if response.data:
            ...
            if existing_pin == clean_pin:
                ...
                st.session_state.db_status = f"🟢 Connected to: '{st.session_state.list_name}'"
                return True
            else:
...
with st.sidebar:
    ...
    if st.session_state.user_passcode.strip() and st.session_state.user_pin.strip() and not st.session_state.auth_error:
        st.markdown("---")
        st.html(f"<h3 style='color: {TEXT_COLOR}; font-family: {FONT_FAMILY};'>📋 Cloud List Selector</h3>")
        
        avail_lists = get_available_lists(st.session_state.user_passcode)
        if not avail_lists:
            avail_lists = [st.session_state.list_name if st.session_state.list_name else "Main List"]
        ...
        if "cloud_list_selector" not in st.session_state or st.session_state.cloud_list_selector not in avail_lists:
            st.session_state.cloud_list_selector = current_active
        ...
        if selected_cloud_list != st.session_state.list_name:
            ...
            load_from_cloud(st.session_state.user_passcode, st.session_state.user_pin, selected_cloud_list)
            st.session_state.current_index = 0
            st.rerun()
        ...
        if st.session_state.show_new_list_input:
            new_list_name = st.text_input("New List Name:", key="new_list_name_sidebar")
            if st.button("Create List", use_container_width=True):
                if new_list_name.strip():
                    st.session_state.list_name = new_list_name.strip()
                    st.session_state.tasks = []
                    st.session_state.cloud_list_selector = new_list_name.strip()
                    st.session_state.show_new_list_input = False
                    save_to_cloud()
                    st.rerun()
            ...
            if isinstance(imported_data, list):
                ...
                if replace_clicked:
                    ...
                    if current_count == 0:
                        ...
                        st.sidebar.error(f"❌ Unable to import: combined count exceeds limit of {LIMIT}.")
                    else:
...
if st.session_state.mode == "adding":
    ...
    with right_col:
        ...
        with st.form(key=f"input_form_{ver_key}", clear_on_submit=True):
            task_text = st.text_input(form_title, value=st.session_state.edit_task_name)             
