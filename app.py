...
if not st.session_state.loaded_from_browser:
    # 1. Create a hidden Streamlit text input with a unique key
    sync_input = st.text_input("sync_transfer", value="", key="sync_transfer_input", label_visibility="collapsed")
    
    # 2. If the JavaScript has typed the data into the input field, read it
    if sync_input:
        try:
        ...
        # 3. Inject JavaScript to fetch from localStorage, inject into our hidden input, and fire the change event
        st.html(
            """
            <script>
            // Find the hidden input element generated by Streamlit
            const checkExist = setInterval(() => {
                const inputs = window.parent.document.querySelectorAll('input[aria-label="sync_transfer"]');
                if (inputs.length > 0) {
                    clearInterval(checkExist);
                    const inputEl = inputs[0];
                    const saved = localStorage.getItem("executive_tasks_list") || "[]";
                    
                    // Native value setter helper to bypass React's virtual DOM listeners
                    const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, "value").set;
                    nativeInputValueSetter.call(inputEl, saved);
                    
                    // Dispatch event so Streamlit registers the typing action and transmits via websocket
                    const event = new Event('input', { bubbles: true });
                    inputEl.dispatchEvent(event);
                    
                    // Blur/Submit simulation
